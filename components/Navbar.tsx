...
const Navbar: React.FC<NavbarProps> = ({ customLogo }) => {
  ...
  // Scroll to a section. If we're not on the home page, navigate to home then scroll.
  const goToSection = (id: string, targetPath: string = '/') => {
    if (location.pathname === targetPath) {
      ...
      // After navigation, the browser jumps to the anchor; we use a tiny delay to smooth it
      setTimeout(() => {
        const element = document.getElementById(id);
        if (element) {
          const headerOffset = 100;
